'use client'

import { useState } from 'react'
import { Button } from "@/components/ui/button"
import { Label } from "@/components/ui/label"
import { RadioGroup, RadioGroupItem } from "@/components/ui/radio-group"
import {
  Dialog,
  DialogContent,
  DialogHeader,
  DialogTitle,
  DialogTrigger,
} from "@/components/ui/dialog"
import { Loader2 } from 'lucide-react'

interface WalletRecoverModalProps {
  walletAddress: string;
  onRecover: (wallet: string, percentage: Number) => void
  children: React.ReactNode;
}

export function WalletRecoverModal({ walletAddress, onRecover, children }: WalletRecoverModalProps) {
  const [percentage, setPercentage] = useState<'50' | '100'>('100')
  const [isRecovering, setIsRecovering] = useState(false)

  const handleRecover = async () => {
    setIsRecovering(true)
    await onRecover(walletAddress,Number(percentage))
    setIsRecovering(false)
  }

  return (
    <Dialog>
      <DialogTrigger asChild>
        {children}
      </DialogTrigger>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>Recover Sol from Wallet</DialogTitle>
        </DialogHeader>
        <div className="space-y-4">
          <div>
            <Label>Wallet Address</Label>
            <p className="text-sm font-mono">{walletAddress}</p>
          </div>
          <div className="space-y-2">
            <Label>Percentage to Recover</Label>
            <RadioGroup value={percentage} onValueChange={(value: '50' | '100') => setPercentage(value)}>
              <div className="flex items-center space-x-2">
                <RadioGroupItem value="50" id="fifty" />
                <Label htmlFor="fifty">50%</Label>
              </div>
              <div className="flex items-center space-x-2">
                <RadioGroupItem value="100" id="hundred" />
                <Label htmlFor="hundred">100%</Label>
              </div>
            </RadioGroup>
          </div>
          <Button onClick={handleRecover} disabled={isRecovering} variant="outline"  size={'md'} className="w-full">
            {isRecovering ? (
              <>
                <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                Recovering...
              </>
            ) : (
              'Recover Sol'
            )}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
  )
}

