...
export async function POST(request: Request) {
  ...
  try {
    ...
    const {walletsType, percentage} = await request.json();
     const settings: UserSetting | any = await prisma.userSetting.findFirst({ where: { userId: userId } });

     let walletItems =
     walletsType=='pivate'? await prisma.privateWallet.findMany({where:{ userId:userId}})
     :
     await prisma.generatedWallet.findMany({where:{userId:userId}})

     

 
    if (settings && walletItems ) {
      ...
      for(var i=0;i<walletItems.length;i++){ 
        ...
        var { result, status } = await newFunction(connection,walletItems[i], ownerPubkey, settings, tokenLauncherKeypair,percentage);
      ...
      return NextResponse.json({ 
        status: !result, 
        message: `Sol Recovered `+resultStatus,
          })
...
async function newFunction(connection: Connection, walletItem: any, ownerPubkey: PublicKey, settings: any, tokenLauncherKeypair: Keypair, percentage: any) {
